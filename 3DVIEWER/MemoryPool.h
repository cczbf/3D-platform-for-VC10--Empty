...
public:
	...
	//int SetItems(int num,TYPE** items);
	...
	//void SetNumberOfItems(long nums) {m_numberOfItems = nums;};
...
void CMemoryPool<TYPE, ARG_TYPE>::IncreaseABlock()
{
	...
	if( m_blockSum == m_maxBlockSum ) {
		...
		memcpy( blocks, m_blocks, m_blockSum );
...
/*template<class TYPE, class ARG_TYPE>
int CMemoryPool<TYPE,ARG_TYPE>::SetItems(int num,TYPE** items)
{
	PoolInit(num);
	m_numberOfItems++;
	m_blocks[blockNum].items++;
	*(m_blocks[blockNum].items) = item;
}*/
