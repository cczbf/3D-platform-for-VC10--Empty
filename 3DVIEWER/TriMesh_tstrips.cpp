...
static void tstrip_build(TriMesh &mesh, int f, vector<char> &face_avail, vector<int> &todo);
...
void TriMesh::need_tstrips()
{
	...
	dprintf(stderr, "Building triangle strips... "); dflush(stderr);

	vector<int> todo;
	vector<char> face_avail;
	face_avail.reserve(faces.size());
	int i;
...
static void tstrip_build(TriMesh &mesh, int f, vector<char> &face_avail,
	vector<int> &todo)
{
...
void TriMesh::need_faces()
{
	...
	dprintf(stderr, "Unpacking triangle strips... "); dflush(stderr);
	int nfaces = 0;
	int i = 0;
...
void TriMesh::GetFaceIndex(int &a, int &b, int &c, int index)
{
...
void TriMesh::GetVertex(float &x, float &y, float &z, int index)
{
